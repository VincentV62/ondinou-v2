import { useState } from "react";
import { motion } from "framer-motion";
import { useNavigate } from "react-router-dom";
import { supabase } from "@/integrations/supabase/client";
import dinouLogo from "@/assets/dinou-logo.png";
import { Mail, Phone } from "lucide-react";

const AuthPage = () => {
  const navigate = useNavigate();
  const [mode, setMode] = useState<"login" | "signup">("login");
  const [method, setMethod] = useState<"email" | "phone" | null>(null);
  const [email, setEmail] = useState("");
  const [phone, setPhoneVal] = useState("");
  const [password, setPassword] = useState("");
  const [firstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");

  const handleEmailAuth = async () => {
    setLoading(true);
    setError("");
    if (mode === "signup") {
      const { error } = await supabase.auth.signUp({
        email,
        password,
        options: {
          data: { first_name: firstName, last_name: lastName },
          emailRedirectTo: window.location.origin,
        },
      });
      if (error) setError(error.message);
      else setMessage("Vérifie ton email pour confirmer ton compte !");
    } else {
      const { error } = await supabase.auth.signInWithPassword({ email, password });
      if (error) setError(error.message);
      else navigate("/reservation");
    }
    setLoading(false);
  };

  const handleGoogleAuth = async () => {
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: window.location.origin + "/reservation" },
    });
  };

  return (
    <div className="flex flex-col items-center min-h-screen bg-background px-6 py-8">
      <motion.img
        src={dinouLogo}
        alt="Dinou"
        className="w-32 h-32 sm:w-40 sm:h-40 object-contain"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
      />
      {/* Speech bubble */}
      <motion.div
        className="relative mt-3 mx-4 max-w-xs bg-card border border-border rounded-2xl px-5 py-3 shadow-md"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.3 }}
      >
        <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-card border-l border-t border-border rotate-45" />
        <p className="text-sm text-foreground font-body text-center relative z-10">
          {mode === "login"
            ? "Salut, je suis Dinou et je vais te trouver le resto dont tu as besoin en quelques clics !"
            : "Rejoins ONDINOU et trouve ton resto idéal !"}
        </p>
      </motion.div>

      {message && (
        <div className="mt-4 p-3 rounded-lg bg-secondary/30 text-foreground text-sm font-body max-w-xs text-center">
          {message}
        </div>
      )}

      <div className="mt-6 w-full max-w-xs space-y-3">
        {/* Google */}
        <motion.button
          onClick={handleGoogleAuth}
          className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-card text-card-foreground border border-border font-body hover:border-accent/50 transition-colors"
          whileTap={{ scale: 0.97 }}
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
          Continuer avec Google
        </motion.button>

        {/* Method selection */}
        {!method && (
          <>
            <motion.button
              onClick={() => setMethod("email")}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-card text-card-foreground border border-border font-body hover:border-accent/50 transition-colors"
              whileTap={{ scale: 0.97 }}
            >
              <Mail className="w-5 h-5" /> Continuer avec Email
            </motion.button>
            <motion.button
              onClick={() => setMethod("phone")}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-card text-card-foreground border border-border font-body hover:border-accent/50 transition-colors"
              whileTap={{ scale: 0.97 }}
            >
              <Phone className="w-5 h-5" /> Continuer avec Téléphone
            </motion.button>
          </>
        )}

        {/* Email form */}
        {method === "email" && (
          <motion.div
            className="space-y-3"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          >
            {mode === "signup" && (
              <div className="grid grid-cols-2 gap-2">
                <input
                  type="text"
                  placeholder="Prénom"
                  value={firstName}
                  onChange={(e) => setFirstName(e.target.value)}
                  className="px-3 py-2.5 rounded-xl bg-card text-card-foreground border border-border font-body text-sm focus:outline-none focus:ring-2 focus:ring-accent"
                />
                <input
                  type="text"
                  placeholder="Nom"
                  value={lastName}
                  onChange={(e) => setLastName(e.target.value)}
                  className="px-3 py-2.5 rounded-xl bg-card text-card-foreground border border-border font-body text-sm focus:outline-none focus:ring-2 focus:ring-accent"
                />
              </div>
            )}
            <input
              type="email"
              placeholder="Email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-card text-card-foreground border border-border font-body text-sm focus:outline-none focus:ring-2 focus:ring-accent"
            />
            <input
              type="password"
              placeholder="Mot de passe"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-card text-card-foreground border border-border font-body text-sm focus:outline-none focus:ring-2 focus:ring-accent"
            />
            {error && <p className="text-destructive text-xs font-body">{error}</p>}
            <button
              onClick={handleEmailAuth}
              disabled={loading}
              className="w-full py-3 rounded-full bg-accent text-accent-foreground font-heading font-semibold shadow-md disabled:opacity-50"
            >
              {loading ? "..." : mode === "login" ? "Se connecter" : "Créer mon compte"}
            </button>
          </motion.div>
        )}

        {method === "phone" && (
          <motion.div
            className="space-y-3"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          >
            <input
              type="tel"
              placeholder="+33 6 12 34 56 78"
              value={phone}
              onChange={(e) => setPhoneVal(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-card text-card-foreground border border-border font-body text-sm focus:outline-none focus:ring-2 focus:ring-accent"
            />
            <p className="text-xs text-muted-foreground font-body">La connexion par téléphone sera bientôt disponible.</p>
          </motion.div>
        )}
      </div>

      <button
        onClick={() => { setMode(mode === "login" ? "signup" : "login"); setMethod(null); setError(""); }}
        className="mt-6 text-sm text-accent font-body underline"
      >
        {mode === "login" ? "Pas encore de compte ? Inscris-toi" : "Déjà un compte ? Connecte-toi"}
      </button>

      <button
        onClick={() => navigate("/reservation")}
        className="mt-3 text-xs text-muted-foreground font-body"
      >
        Continuer sans compte →
      </button>

      <button
        onClick={() => navigate("/restaurant-signup")}
        className="mt-2 text-xs font-body font-semibold text-[hsl(var(--night-blue))] underline"
      >
        Je suis restaurateur
      </button>

      <button
        onClick={() => navigate("/creator")}
        className="mt-3 text-[10px] text-muted-foreground/50 font-body hover:text-muted-foreground transition-colors"
      >
        page créateur
      </button>
    </div>
  );
};

export default AuthPage;
